...
function start_mesos_master() {
    ...
    MASTER=$(sudo docker run -P=true  -i -t -d -dns $NAMESERVER_IP -h master mesos-master:$MESOS_VERSION)
...
function start_mesos_workers() {
    for i in `seq 1 $NUM_WORKERS`; do
        ...
        hostname="worker${i}"
        WORKER=$(sudo docker run -P=true  -d -dns $NAMESERVER_IP -h $hostname mesos-worker:${MESOS_VERSION} ${MASTER_IP} ${MASTER_IP}:5050)
...
function print_cluster_info() {
    ...
    echo ""
    echo "start Spark Shell:          sudo docker run -P=true  -i -t -dns $NAMESERVER_IP -h spark-client spark-shell-mesos:0.7.3 $MASTER_IP"
    echo "start Shark Shell:          sudo docker run -P=true  -i -t -dns $NAMESERVER_IP -h shark-client shark-shell-mesos:0.7.0 $MASTER_IP"
    echo ""
